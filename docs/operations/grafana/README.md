...
The row expects all three services (`openai-livepeer-bridge`, `livepeer-payment-library`'s daemon, `livepeer-byoc`'s worker) to be scraped by the same Prometheus. Cross-environment scrapes also work as long as the metric names line up.
