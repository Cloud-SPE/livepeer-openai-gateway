...
describe('admin routes — validation rejections', () => {
  it('GET /admin/customers rejects an out-of-range limit', async () => {
    ...
    try {
      const res = await server.app.inject({
        method: 'GET',
        url: '/admin/customers?limit=9999',
        headers: auth('admin'),
      });
  ...
  it('GET /admin/customers/:id/api-keys rejects non-uuid id', async () => {
    ...
    try {
      const res = await server.app.inject({
        method: 'GET',
        url: '/admin/customers/not-a-uuid/api-keys',
        headers: auth('admin'),
      });
      expect(res.statusCode).toBe(400);
    } finally {
  ...
  it('POST /admin/customers/:id/api-keys rejects missing body', async () => {
    ...
    try {
      const res = await server.app.inject({
        method: 'POST',
        url: `/admin/customers/${customer.id}/api-keys`,
        headers: { ...auth('admin'), 'content-type': 'application/json' },
        payload: '{}',
      });
      expect(res.statusCode).toBe(400);
  ...
  it('GET /admin/audit silently drops malformed from/to dates (treated as missing)', async () => {
    ...
    try {
      const res = await server.app.inject({
        method: 'GET',
        url: '/admin/audit?from=not-a-date&to=also-not&limit=10',
        headers: auth('admin'),
      });
  ...
  it('GET /admin/reservations rejects invalid state', async () => {
    ...
    try {
      const res = await server.app.inject({
        method: 'GET',
        url: '/admin/reservations?state=bogus',
        headers: auth('admin'),
      });
      expect(res.statusCode).toBe(400);
    } finally {
  ...
  it('GET /admin/topups rejects invalid customer_id (non-uuid)', async () => {
    ...
    try {
      const res = await server.app.inject({
        method: 'GET',
        url: '/admin/topups?customer_id=not-a-uuid',
        headers: auth('admin'),
      });
      expect(res.statusCode).toBe(400);
    } finally {
  ...
  it('GET /admin/nodes/:id/events rejects out-of-range limit', async () => {
    ...
    try {
      const res = await server.app.inject({
        method: 'GET',
        url: '/admin/nodes/foo/events?limit=99999',
        headers: auth('admin'),
      });
      expect(res.statusCode).toBe(400);
    } finally {
  ...
  it('GET /admin/reservations with cursor exercises the cursor branch', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await reservationsRepo.insertReservation(pg.db, {
        ...
        workId: `cur-w-${i}`,
        kind: 'prepaid',
        amountUsdCents: BigInt(100 + i),
        state: 'open',
        createdAt: new Date(`2026-04-${20 + i}T10:00:00Z`),
      });
    }
    const server = await buildServer();
    try {
      const page1 = await server.app.inject({
        method: 'GET',
        url: '/admin/reservations?state=open&limit=2',
        headers: auth('admin'),
      });
