...
export class PortalUsage extends LitElement {
  ...
  async _refresh() {
    try {
      ...
      showToast({ kind: 'error', message: err instanceof Error ? err.message : 'Failed to load usage.' });
    }
  }
    ...
    return html`
      <div class="page-header">
        ...
        <div style="display: flex; gap: var(--space-2)">
          ${['day', 'model', 'capability'].map((g) => html`
            <bridge-button
      ...
      ${data ? html`
        <p class="muted text-sm">
          ${data.totals.requests} requests ·
          ${data.totals.prompt_tokens.toLocaleString()} prompt tokens ·
          ${data.totals.completion_tokens.toLocaleString()} completion tokens ·
  ...
  _columns() {
    const headerForGroup = this._groupBy === 'day' ? 'Date' : this._groupBy === 'model' ? 'Model' : 'Capability';
    return [
      ...
      { field: 'completion_tokens', header: 'Completion', render: (r) => r.completion_tokens.toLocaleString() },
