...
export class AdminConfig extends LitElement {
  ...
  async _refresh() {
    try { await configService.refresh(); }
    catch (err) { showToast({ kind: 'error', message: err instanceof Error ? err.message : 'Failed to load nodes.yaml.' }); }
  }
    ...
    return html`
      ...
      <section style="background: var(--surface-1); border: 1px solid var(--border-1); border-radius: var(--radius-lg); padding: var(--space-5); margin-bottom: var(--space-5)">
        <dl style="display: grid; grid-template-columns: max-content 1fr; gap: var(--space-2) var(--space-4); margin: 0">
          <dt class="muted text-sm">Path</dt><dd class="mono text-xs" style="margin: 0">${s.path}</dd>
          <dt class="muted text-sm">SHA-256</dt><dd class="mono text-xs" style="margin: 0">${s.sha256}</dd>
          <dt class="muted text-sm">Modified</dt><dd style="margin: 0">${new Date(s.mtime).toLocaleString()}</dd>
          <dt class="muted text-sm">Size</dt><dd style="margin: 0">${s.size_bytes} bytes</dd>
        </dl>
      </section>
  ...
  _loadedColumns() {
    return [
      { field: 'id', header: 'ID' },
      { field: 'url', header: 'URL', render: (n) => html`<span class="mono text-xs">${n.url}</span>` },
      { field: 'enabled', header: 'Enabled', render: (n) => n.enabled ? 'yes' : html`<span class="muted">no</span>` },
