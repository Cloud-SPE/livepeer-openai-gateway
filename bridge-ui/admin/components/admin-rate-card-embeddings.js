...
export class AdminRateCardEmbeddings extends LitElement {
  ...
  render() {
    ...
    return html`
        ...
        <div style="display:grid; gap: var(--space-3);">
          <label
            ...
            <select
              ...
              @change=${(e) => this._set('is_pattern', e.target.value === 'pattern')}
              style="width:100%"
            >
          ...
          <label
            >${this._form.is_pattern ? 'Pattern' : 'Model'}
            <input
              type="text"
              maxlength="256"
              style="width:100%"
              .value=${this._form.model_or_pattern}
              placeholder=${this._form.is_pattern
                ? 'e.g. text-embedding-*'
          ...
          <label
            ...
            <input
              ...
              inputmode="decimal"
              style="width:100%"
              .value=${this._form.usd_per_million_tokens}
          ...
          ${this._form.is_pattern
            ? html`<label
                >Sort order<input
                  ...
                  @input=${(e) => this._set('sort_order', e.target.value)}
              /></label>`
            : ''}
      ...
      <bridge-confirm-dialog
        ...
        body="Customers requesting this model will get model_not_found until you re-add it."
  ...
  _cols() {
    return [
      ...
      {
        field: 'usd_per_million_tokens',
        header: '$/M tok',
        render: (r) => Number(r.usd_per_million_tokens).toFixed(4),
      },
  ...
  _emptyForm() {
    return { model_or_pattern: '', is_pattern: false, usd_per_million_tokens: '', sort_order: 100 };
  }
  _openAdd() {
    this._form = this._emptyForm();
    this._err = '';
  ...
  _preview() {
    const cands = (this.entries.value?.entries ?? [])
  ...
  _submitAdd = async () => {
    ...
    if (!f.model_or_pattern.trim()) {
      this._err = 'Model / pattern required.';
      return;
    }
    if (!f.usd_per_million_tokens.toString().trim()) {
      this._err = 'Price required.';
      return;
    }
    this._busy = true;
    this._err = '';
    try {
      const body = {
        ...
        usd_per_million_tokens: f.usd_per_million_tokens,
      };
...
if (!customElements.get('admin-rate-card-embeddings'))
  customElements.define('admin-rate-card-embeddings', AdminRateCardEmbeddings);
