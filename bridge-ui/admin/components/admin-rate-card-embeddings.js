...
export class AdminRateCardEmbeddings extends LitElement {
  static properties = {
  ...
  constructor() {
    super();
    this.entries = new ObservableController(this, rateCardService.embeddings$);
  ...
  async connectedCallback() {
    ...
    rateCardService.fetchEmbeddings().catch((e) => showToast({ kind: 'error', message: e.message }));
  }
  render() {
    const entries = this.entries.value?.entries ?? null;
    return html`
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: var(--space-3);">
        <h2 style="margin:0;">Embeddings — model → USD per 1M tokens</h2>
      ...
      `}
      <bridge-dialog ?open=${this._addOpen} heading="Add embeddings entry" @bridge-close=${() => this._closeAdd()}>
        <div style="display:grid; gap: var(--space-3);">
          ...
          <label>${this._form.is_pattern ? 'Pattern' : 'Model'}
            <input type="text" maxlength="256" style="width:100%" .value=${this._form.model_or_pattern}
              placeholder=${this._form.is_pattern ? 'e.g. text-embedding-*' : 'e.g. text-embedding-3-large'}
          ...
          <label>USD per 1M tokens
            <input type="text" inputmode="decimal" style="width:100%" .value=${this._form.usd_per_million_tokens}
              @input=${(e) => this._set('usd_per_million_tokens', e.target.value)}/>
          </label>
  ...
  _cols() {
    return [
      ...
      { field: 'usd_per_million_tokens', header: '$/M tok', render: (r) => Number(r.usd_per_million_tokens).toFixed(4) },
  ...
  _emptyForm() { return { model_or_pattern: '', is_pattern: false, usd_per_million_tokens: '', sort_order: 100 }; }
  ...
  _submitAdd = async () => {
    ...
    if (!f.usd_per_million_tokens.toString().trim()) { this._err = 'Price required.'; return; }
    this._busy = true; this._err = '';
    try {
      const body = { model_or_pattern: f.model_or_pattern.trim(), is_pattern: f.is_pattern, usd_per_million_tokens: f.usd_per_million_tokens };
      if (f.is_pattern) body.sort_order = Number(f.sort_order) || 100;
      await rateCardService.createEmbeddings(body);
      this._addOpen = false;
  ...
  _confirmDelete = async () => {
    ...
    this._busy = true;
    try {
      await rateCardService.deleteEmbeddings(id);
...
if (!customElements.get('admin-rate-card-embeddings')) customElements.define('admin-rate-card-embeddings', AdminRateCardEmbeddings);
