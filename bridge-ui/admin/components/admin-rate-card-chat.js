...
export class AdminRateCardChat extends LitElement {
  ...
  async connectedCallback() {
    ...
    await Promise.all([
      rateCardService.fetchChatTiers().catch((e) => showToast({ kind: 'error', message: e.message })),
      rateCardService.fetchChatModels().catch((e) => showToast({ kind: 'error', message: e.message })),
    ]);
  }
  ...
  render() {
    ...
    return html`
      <section style="margin-bottom: var(--space-5);">
        <h2>Tier prices</h2>
        ${tiers === null ? html`<bridge-spinner></bridge-spinner>` : html`
          <bridge-table
            .columns=${this._tierColumns()}
            .rows=${tiers}
          ></bridge-table>
        `}
      </section>
      ...
      >
        <div style="display:grid; gap: var(--space-3);">
          <label>
            Type
            <select .value=${this._form.is_pattern ? 'pattern' : 'exact'} @change=${(e) => this._setField('is_pattern', e.target.value === 'pattern')} style="width:100%">
          ...
          <label>
            ...
            <input
              type="text" maxlength="256" style="width:100%"
            ...
            />
            ${this._form.is_pattern && this._form.model_or_pattern ? html`
              <p class="muted text-sm" style="margin: var(--space-1) 0 0;">
                ${this._patternPreview()}
              </p>` : ''}
          </label>
          <label>
            Tier
            <select .value=${this._form.tier} @change=${(e) => this._setField('tier', e.target.value)} style="width:100%">
              ${TIERS.map((t) => html`<option value=${t}>${t}</option>`)}
            </select>
          </label>
          ${this._form.is_pattern ? html`
            <label>
              Sort order (lower wins on ambiguous matches)
              <input
                type="number" min="0" max="10000" style="width:100%"
      ...
      <bridge-confirm-dialog
        ...
        confirm-label="Delete" cancel-label="Cancel" danger
        ?loading=${this._busy}
        @bridge-confirm=${this._confirmDelete}
        @bridge-cancel=${() => { this._deleteId = null; }}
        @bridge-close=${() => { this._deleteId = null; }}
      ></bridge-confirm-dialog>

      <bridge-dialog
        ?open=${this._editingTier !== null}
        heading=${`Edit ${this._editingTier ?? ''} tier price`}
        @bridge-close=${() => { this._editingTier = null; }}
      >
        <div style="display:grid; gap: var(--space-3);">
          <label>
            Input price (USD per 1M tokens)
            <input
              type="text" inputmode="decimal" style="width:100%"
              .value=${this._tierForm.input_usd_per_million}
              @input=${(e) => this._tierForm = { ...this._tierForm, input_usd_per_million: e.target.value }}
            />
          </label>
          <label>
            ...
            <input
              ...
              @input=${(e) => this._tierForm = { ...this._tierForm, output_usd_per_million: e.target.value }}
            />
          </label>
        </div>
        <div slot="actions">
          <bridge-button variant="ghost" @click=${() => { this._editingTier = null; }}>Cancel</bridge-button>
  ...
  _tierColumns() {
    return [
      ...
      { field: 'input_usd_per_million', header: 'Input ($/M tok)', render: (r) => Number(r.input_usd_per_million).toFixed(4) },
      { field: 'output_usd_per_million', header: 'Output ($/M tok)', render: (r) => Number(r.output_usd_per_million).toFixed(4) },
      { field: 'updated_at', header: 'Updated', render: (r) => new Date(r.updated_at).toLocaleString() },
      { field: 'actions', header: '', render: (r) => html`<bridge-button variant="ghost" @click=${() => this._openTierEdit(r)}>Edit</bridge-button>` },
    ];
  }

  _modelColumns() {
    return [
      { field: 'model_or_pattern', header: 'Model / Pattern', render: (r) => html`<span>${r.model_or_pattern}${r.is_pattern ? html` <span class="muted">(glob)</span>` : ''}</span>` },
      { field: 'tier', header: 'Tier', render: (r) => r.tier },
      { field: 'sort_order', header: 'Sort', render: (r) => r.is_pattern ? r.sort_order : '—' },
      { field: 'created_at', header: 'Created', render: (r) => new Date(r.created_at).toLocaleDateString() },
      { field: 'actions', header: '', render: (r) => html`<bridge-button variant="ghost" @click=${() => this._askDelete(r.id)}>Delete</bridge-button>` },
    ];
  }
  ...
  _setField(name, value) { this._form = { ...this._form, [name]: value }; }
  _patternPreview() {
    const candidates = (this.models.value?.entries ?? []).filter((e) => !e.is_pattern).map((e) => e.model_or_pattern);
  ...
  _submitAdd = async () => {
    const f = this._form;
    if (!f.model_or_pattern.trim()) { this._err = 'Model / pattern is required.'; return; }
  ...
  _askDelete(id) { this._deleteId = id; }
